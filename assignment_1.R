...
random_dosetox_curves <- function(ndose = 4, nsim = 100, mtd = 0.2){
  ...
  sim1 <- function(J = doses){
    ...
    found_mtd <- suppressWarnings(max(max(which((probs - mtd) < 0)), 0))
...
matplot(t(random_dosetox_curves(ndose = 4, nsim = 100)[, 1:4]), type = "l", xlab = "Dose", ylab = "DLT") 
...
#Set seed
set.seed(545)
#Set dose toxicity curves
dosetox <- random_dosetox_curves()
...
for(i in 1:100){
  ...
  table2 <- non_bench(theta = 0.2, npat = 24, nsim = 1000, dltcuts = dosetox[i, 1:4])
  doseselect <- c(sum(table2 == 1), sum(table2 == 2), sum(table2 == 3), sum(table2 == 4))/ (1000)
  vals[i, ] <- c(doseselect, dosetox[i, 5])
...
#Get final dosages
table5 <- replicate(1000, i33()$dose)
...
for(i in 1:100){
  table5.2 <- replicate(1000, i33(mtd = 0.2, n = 3, ncohort = 8, dltprobs = as.vector(dosetox[i, 1:4]))$dose)
  doseselect <- c(sum(table5.2 == 1), sum(table5.2 == 2), sum(table5.2 == 3), sum(table5.2 == 4))/ (24 * 1000)
  vals[i, 1:5] <- c(doseselect, dosetox[i, 5])
}
